...
function drawExplosion(explosion) {
	...
	for (let i = 0; i < numPts; i++) {
		...
		const p = p5.Vector.add(explosion.pos, p5.Vector.random2D().mult(randomGaussian(radius, radius * 0.3)))
		vertex(p.x, p.y)
	}
	endShape(CLOSE)
	pop()
...
function updateShip() {
	...
	if (keyIsDown(UP_ARROW)) {
		if (world.ship.fuel > 0) {
			...
			world.particles.push(createThrustParticle(world.ship.pos, thrustVec.copy().rotate(randomGaussian(PI, PI / 20)).setMag(0.9)))
			if (world.state.type === "landed") {
				world.state = {
					type: "flying"
				}
...
function createLandingPads(palette) {
	...
	return zipWith(baseNames, [0.2, 0.8], (name, frac) => createOnePad({
		name,
		frac
	}));
}
